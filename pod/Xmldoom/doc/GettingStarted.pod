...
example.  (It should be noted that the example/BookStore that comes with the main 
distribution is not the same as the example layed out here.  That exists primarily for
the automatic unittests and violates many of the "best practices" described here.)
...
and Creole [2], which means that we can use their tools.  Both projects provide tools
to either examine your database and produce XML, or to read the XML and produce a
list of SQL CREATE statements.  See this document on using Apache Torque for this purpose
with Xmldoom: L<Xmldoom::doc::UsingTorque>.
...
Here is the simplest possible BookStore/Object.pm code:
  ...
  BEGIN
  {
	...
	$DATABASE = Xmldoom::Definition::parse_database_uri("database.xml");

	# read the object description
	Xmldoom::Definition::parse_object_uri( $DATABASE, "objects.xml" );
...
For all the objects in defined in the object definition described above, you need
